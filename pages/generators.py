...
# tab_ai, tab_merge, tab_blend, tab_nick, tab_unique, tab_neutral = st.tabs([
#     "AI Name Analysis",
#     "Name Merger",
#     "Parent Name Blender",
#     "Nickname Generator",
...
# # --- TAB 2: NAME MERGER ---
# with tab_merge:
#     st.header("Creative Name Merger")
...
# # --- TAB 3: PARENT NAME BLENDER ---
# with tab_blend:
#     st.header("Parent Name Blender")
#     st.markdown(
#         "Blend two parent names and discover baby name ideas that feel like family.")
#     with st.container(border=True):
#         col_a, col_b = st.columns(2)
#         with col_a:
#             parent_one = st.text_input("Parent one name", "Sophia")
#         with col_b:
#             parent_two = st.text_input("Parent two name", "Liam")
#         vibe_choice = st.selectbox("Preferred vibe", list(VIBE_OPTIONS.keys()))
#         gender_choice = st.selectbox(
#             "Baby name style", list(GENDER_FILTERS.keys()))
#     if st.button("Find family-fit names", use_container_width=True):
...
# # --- TAB 6: GENDER-NEUTRAL FINDER ---
# with tab_neutral:
#     st.header("Gender-Neutral Name Finder")
#     st.markdown(
#         "Discover names that are used for multiple genders in our dataset.")
...
#         gender_distribution = neutral_df.groupby(["name", "gender"], observed=True)[
#             "count"].sum().reset_index()
#         fig_gender_neutral = px.bar(
#             gender_distribution, x="name", y="count", color="gender",
#             title="Gender Distribution of Neutral Names", barmode="stack",
#             color_discrete_map={"F": "lightpink",
#                                 "M": "lightblue", "U": "lightgray"},
#         )
#         st.plotly_chart(fig_gender_neutral, use_container_width=True)
#     else:
...
with tab_neutral:
    ...
    if not neutral_name_list.empty:
        ...
        display_df = display_df[['Name', 'Count', 'Female %', 'Male %']]
